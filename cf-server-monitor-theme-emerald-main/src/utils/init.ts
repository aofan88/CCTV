  ...
  turnstileScriptPromise = new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      ...
      if (api)
        ...
        reject(new Error('Turnstile SDK 載入失敗'))
    }
    script.onerror = () => reject(new Error('Turnstile SDK 載入失敗'))
  ...
  return new Promise((resolve, reject) => {
    ...
    overlay.innerHTML = `
      <div class="w-full max-w-sm rounded-md border border-emerald-600/15 bg-background p-5 shadow-2xl">
        <div class="mb-1 text-base font-semibold">存取驗證</div>
        <div class="mb-5 text-sm text-muted-foreground">完成 Cloudflare 驗證後繼續載入監控數據。</div>
    ...
    if (!container) {
      overlay.remove()
      reject(new Error('Turnstile 容器創建失敗'))
      return
    }
    ...
    widgetId = api.render(container, {
      ...
      'expired-callback': () => {
        cleanup()
        reject(new Error('Turnstile 驗證已過期'))
      },
    })
  })
}
