...
const nodePriceText = computed(() => {
  ...
  if (Number(data.value.price) <= 0)
    return appStore.lang === 'zh-CN' ? '免費' : 'Free'
...
const monthlyAverageCostText = computed(() => {
  if (!data.value)
    return '-'
  if (data.value.price_configured === false)
    return appStore.lang === 'zh-CN' ? '不適用' : 'N/A'
  if (Number(data.value.price) <= 0)
    return appStore.lang === 'zh-CN' ? '免費' : 'Free'
...
const hardwareInfo = computed<InfoItem[]>(() => [
  ...
  { label: '架構', value: data.value?.arch ?? '-', icon: 'icon-park-outline:application-two' },
  { label: '核心', value: data.value?.kernel_version ?? '-', icon: 'icon-park-outline:server' },
  { label: 'GPU', value: gpuDisplayText.value, icon: 'icon-park-outline:video-one' },
])

const systemInfo = computed<InfoItem[]>(() => [
  { label: '操作系統', value: data.value?.os ?? '-', icon: 'icon-park-outline:computer' },
  { label: '啟動時間', value: formatDateTime(data.value?.boot_time), icon: 'icon-park-outline:time' },
  { label: '運行時間', value: formatUptime(data.value?.uptime ?? 0), icon: 'icon-park-outline:timer' },
  { label: '最後上報', value: formatDateTime(data.value?.time), icon: 'icon-park-outline:time' },
])

const storageInfo = computed<InfoItem[]>(() => [
  { label: '記憶體', value: formatBytes(data.value?.mem_total ?? 0), icon: 'icon-park-outline:memory' },
  { label: '記憶體交換', value: formatBytes(data.value?.swap_total ?? 0), icon: 'icon-park-outline:switch' },
  { label: '硬碟', value: formatBytes(data.value?.disk_total ?? 0), icon: 'icon-park-outline:hard-disk' },
])
          ...
          <div class="gap-3 grid grid-cols-1 sm:grid-cols-2">
            ...
            >
              ...
              <div class="flex min-w-0 gap-2 items-center">
                <img
                  v-if="item.label === '操作系統'" :src="getOSImage(data.os, data.source_index)" :alt="getOSName(data.os)"
            ...
            <div class="relative min-w-0 overflow-hidden rounded-sm bg-slate-500/5 p-2">
              ...
              <div class="relative flex flex-col gap-1.5">
                <div class="flex gap-1 items-center text-muted-foreground">
                  ...
                  <span class="text-xs sm:text-sm">總流量</span>
                  <div class="flex-1" />
