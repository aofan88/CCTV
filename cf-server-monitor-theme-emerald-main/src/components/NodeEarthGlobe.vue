...
const regionRates = computed<Map<string, RegionRate>>(() => {
  ...
  // 始終使用 nodesStore.nodes 繞過 earthNodes 60s 節流，使速率即時更新
...
// iOS Safari 對 cobe 內部 marker anchor 的 DOM/style 行為不穩定，
// overlay 改為組件內自行投影定位，避免回落到容器左上角。
...
// 從各地區匯聚到用戶目前位置；無用戶座標時回退到 hub 拓撲
...
function buildInitialOptions(): COBEOptions {
  ...
  return {
    ...
    dark: colors.dark,
    diffuse: 1.2,
    mapSamples: 10000, // 地圖採樣點數，預設 16000
...
// phi 收斂/靜止時整幀跳過 globe.update，WebGL + overlay 位置更新雙雙歸零
...
function startGlobe() {
  ...
  syncClusterOverlayPositions()
  // 靜止地球沒有自轉幀，首幀需要在實際尺寸穩定後主動重繪一次。
  requestAnimationFrame(() => {
    updateGlobeFrame()
  })
  // documentVisibility 同步可讀；useElementVisibility 需等 IntersectionObserver 首回調
  // 先按"前臺"啟動，若實際不可見，shouldRender 的 watch 會在下一幀 pause
  syncRafState()
}

// cobe 不會清理自己創建的 wrapper，這裡手動收尾。
function stopGlobe() {
  pauseRaf()
  globe?.destroy()
  globe = null
...
<template>
  <div ref="containerRef" class="relative aspect-square w-full max-w-md mx-auto -translate-y-4 md:max-w-[34rem] md:translate-y-4">
    <canvas
      ref="canvasRef"
