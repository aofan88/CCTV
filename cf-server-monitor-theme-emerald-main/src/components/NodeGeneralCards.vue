...
const totalSpeed = computed(() => {
  ...
  return {
    up: onlineNodes.reduce((sum, node) => sum + (node.net_out || 0), 0),
    down: onlineNodes.reduce((sum, node) => sum + (node.net_in || 0), 0),
  }
})
      ...
      >
        <div class="flex h-full flex-col justify-between gap-1">
          <div class="flex items-start justify-between">
            ...
            <Icon icon="tabler:arrows-up-down" :width="20" :height="20" class="text-slate-500/20 group-hover:text-slate-300 transition-colors" />
          </div>
          <div class="flex flex-wrap items-baseline gap-x-2 gap-y-1 text-[11px] md:text-xs font-bold">
