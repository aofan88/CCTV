...
function printUsage() {
  console.log(`本地接口測試工具\n\n用法：\n  node test/api-check.js [選項]\n\n選項：\n  --base-url=http://localhost:8787       本地服務地址，默認 ${DEFAULT_BASE_URL}\n  --api-secret=xxx                       API_SECRET，用於登錄和可選寫入測試\n  --admin-user=admin                     管理員用戶名，默認 admin\n  --admin-password=xxx                   管理員密碼，默認使用 API_SECRET\n  --server-id=uuid                       指定服務器 ID\n  --timeout=10000                        單個請求超時時間\n\n環境變量同名可用：BASE_URL、API_SECRET、ADMIN_USER、ADMIN_PASSWORD、SERVER_ID、INCLUDE_WRITE、TIMEOUT_MS\n\n說明：\n  默認只執行安全或只讀檢查；重建數據庫、清理歷史、刪除服務器等破壞性接口不會執行。\n  Cloudflare Turnstile 開啟時，只驗證未攜帶 token 會失敗，不嘗試繞過人機驗證。`);
}
...
async function bootstrap() {
  ...
  const unauthenticatedCases = [
    ...
    { name: 'POST /update 公開服務器上報成功', method: 'POST', path: '/update', expectedStatus: 200, body: { id: MOCK_PUBLIC_SERVER_ID, secret: apiSecret, metrics: buildMockMetrics() }, headers: { 'X-Agent-Version': AGENT_VERSION } },
    { name: 'POST /update 隱藏服務器上報成功', method: 'POST', path: '/update', expectedStatus: 200, body: { id: MOCK_HIDDEN_SERVER_ID, secret: apiSecret, metrics: buildMockMetrics() }, headers: { 'X-Agent-Version': AGENT_VERSION } },
    { name: 'GET /api/server 公開服務器（未登錄）', method: 'GET', path: `/api/server?id=${encodeURIComponent(MOCK_PUBLIC_SERVER_ID)}`, expectedStatus: 200 },
    { name: 'GET /api/server 隱藏服務器（未登錄）', method: 'GET', path: `/api/server?id=${encodeURIComponent(MOCK_HIDDEN_SERVER_ID)}`, expectedStatus: 404 },
  ...
  const authenticatedCases = [
    { name: 'GET /api/server 公開服務器（已登錄）', method: 'GET', path: `/api/server?id=${encodeURIComponent(MOCK_PUBLIC_SERVER_ID)}`, expectedStatus: 200 },
    { name: 'GET /api/server 隱藏服務器（已登錄）', method: 'GET', path: `/api/server?id=${encodeURIComponent(MOCK_HIDDEN_SERVER_ID)}`, expectedStatus: 200 },
    { name: 'GET /api/history/all 公開服務器（已登錄）', method: 'GET', path: `/api/history/all?id=${encodeURIComponent(MOCK_PUBLIC_SERVER_ID)}&hours=1`, expectedStatus: 200 },
    { name: 'GET /api/history/all 隱藏服務器（已登錄）', method: 'GET', path: `/api/history/all?id=${encodeURIComponent(MOCK_HIDDEN_SERVER_ID)}&hours=1`, expectedStatus: 200 }
  ];
  ...
  const adminCases = [
    ...
    { name: 'POST /admin/api edit 參數校驗', method: 'POST', path: '/admin/api', expectedStatus: 400, body: { action: 'edit', id: 'invalid-id' } },
    { name: 'POST /admin/api delete 參數校驗', method: 'POST', path: '/admin/api', expectedStatus: 400, body: { action: 'delete', id: 'invalid-id' } },
    { name: 'POST /admin/api batch_delete 參數校驗', method: 'POST', path: '/admin/api', expectedStatus: 400, body: { action: 'batch_delete', ids: [] } },
    { name: 'POST /admin/api save_order 參數校驗', method: 'POST', path: '/admin/api', expectedStatus: 400, body: { action: 'save_order', orders: [] } },
  ...
  if (!state.token) {
    record('skip', '已登錄接口測試', '-', '未登錄成功，跳過需要 Bearer Token 的接口');
  } else {
    console.log('\n━━━ [已登錄] 測試 ━━━\n');
      ...
      await runCase({
        ...
        after: async result => {
          if (c.name === 'POST /admin/api list' && result.ok && result.status === 200) {
            ...
            if (hasVersion) {
              record('pass', 'POST /admin/api list agent_version 校驗', '-', AGENT_VERSION);
            } else {
              record('fail', 'POST /admin/api list agent_version 校驗', '-', '未找到預期版本號');
            }
          }

          if (c.name === 'POST /admin/api add 成功' && result.status === 200 && result.data && result.data.id) {
            state.createdServerId = result.data.id;
            console.log('add 成功，服務器 ID:', state.createdServerId);
          }
        }
      });
    }
