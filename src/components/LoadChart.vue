...
const isInitialLoad = ref(true) // 是否為首次載入（用於控制即時模式下的 NSpin 顯示）
...
async function fetchRecentData() {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : '取得數據失敗'
    remoteData.value = []
  }
  finally {
    loading.value = false
...
const processChartOption = computed(() => ({
  ...
  tooltip: {
    ...
    formatter: (params: unknown) => {
      ...
      let html = `<div style="font-weight:600;margin-bottom:6px;color:${chartThemeColors.value.textSecondary}">${timeStr}</div>`
      html += '<div style="display:flex;flex-direction:column;gap:4px">'
      html += `<div style="display:flex;align-items:center">${colorDot}<span>進程數</span><span style="margin-left:auto;font-weight:600;margin-left:16px">${displayValue}</span></div>`
      html += '</div>'
      return html
    },
  },
...
watch(selectedView, () => {
  isInitialLoad.value = true // 切換視圖時重置首次載入狀態
  fetchData()
})

watch(() => props.uuid, () => {
  remoteData.value = []
  isInitialLoad.value = true // 切換節點時重置首次載入狀態
  fetchData()
})
    ...
    <Spinner :show="loading">
      ...
      <div v-else class="gap-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
        ...
        >
          <template #header>
            <div class="flex items-center justify-between">
              <span class="text-base font-bold">記憶體</span>
        ...
        >
          <template #header>
            <div class="flex items-center justify-between">
              <span class="text-base font-bold">網路</span>
        ...
        >
          <template #header>
            <div class="flex items-center justify-between">
              <span class="text-base font-bold">連線</span>
              <div class="text-xs flex gap-1 items-baseline">
        ...
        >
          <template #header>
            <div class="flex items-center justify-between">
              <span class="text-base font-bold">進程</span>
              <span class="text-xs">
