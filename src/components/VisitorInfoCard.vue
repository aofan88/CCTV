...
const device = ref('檢測中')
const browser = ref('檢測中')
const ip = ref('取得中')
const isp = ref('取得中')
const location = ref('正在定位訪客來源')
const countryCode = ref('')
const visitTime = ref(formatVisitTime(new Date()))
const flagVisible = ref(true)
const expand = ref(false)

const subtitle = computed(() => loading.value ? '檢測中' : location.value || '網路訪客')
...
function detectClient(): VisitorClientData {
  ...
  let detectedDevice = '桌面設備'
  if (ANDROID_REGEX.test(ua))
    detectedDevice = 'Android 手機'
...
async function fetchVisitorGeo(): Promise<VisitorGeoData | null> {
  const loaders = [
    async (): Promise<VisitorGeoData> => {
      ...
      return {
        ip: data.ip,
        isp: data.isp || data.organization || data.asn_organization || '未知運營商',
    ...
    async (): Promise<VisitorGeoData> => {
      ...
      return {
        ip: data.ip,
        isp: data.connection?.isp || data.connection?.org || '未知運營商',
        location: [data.country, data.city || data.region].filter(Boolean).join(' · ') || '未知位置',
        countryCode: data.country_code || '',
      }
    },
    async (): Promise<VisitorGeoData> => {
      ...
      return {
        ip: data.ip,
        isp: data.asn?.org || data.company?.name || data.datacenter?.datacenter || data.asn?.descr || '未知運營商',
        location: [
