...
const FREE_PRICE_REGEX = /^(?:free|免費)$/i
...
const TWO_YEAR_REGEX = /兩年|二年|2\s*(?:years?|yrs?|y)/i
...
export class CorsError extends ApiError {
  ...
  constructor(origin: string, apiIndex?: number) {
    super('跨域請求被瀏覽器攔截', undefined, apiIndex)
    this.name = 'CorsError'
    this.origin = origin
  }
}
...
/** symbol → ISO code（排除 ISO 代碼鍵，按長度降序匹配） */
...
export function adaptServer(server: CfServer, apiIndex: number): AdaptedServer {
  ...
  const ping: Record<string, NodeStatusPing> = {
    ct: pingEntry('電信', server.ping_ct, server.loss_ct),
    cu: pingEntry('聯通', server.ping_cu, server.loss_cu),
    cm: pingEntry('移動', server.ping_cm, server.loss_cm),
  ...
  return {
    client: {
      ...
      group: server.server_group || '預設分組',
      tags: server.tags || '',
      hidden: false,
...
export class CfMonitorApi {
  async getPublicSettings(): Promise<PublicSettings> {
    ...
    // 已登錄時最長可看近 7 天（168h）；多後端聚合模式不支援長曆史，回退到 24h
    ...
    return {
      ...
      sitename: hasMultipleApiBases() ? document.title : first?.site_title || document.title || '潤昇創新 RunSing Innovation',
      theme: 'emerald',
